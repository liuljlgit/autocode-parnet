...
public class ${className}CacheImpl extends BaseServiceImpl<${className}> implements I${className}Cache {
        ...
        if(!CollectionUtils.isEmpty(queryKeys)){
            ...
            if(Objects.nonNull(cacheValues)){
                ...
                if(queryKeys.size() == cacheEntitys.size()){
                    ...
                    List<${className}> dbValues = super.selectBatchIds(queryKeys, emptyErrMsg);
                    if(!CollectionUtils.isEmpty(dbValues)){
    ...
    public PageBean<${className}> cacheSelectPage(${className} query) {
        ...
        PageBean<${className}> cachePageBean = (PageBean<${className}>)redisTemplate.opsForValue().get(pageBeanKey);
        ...
        List<Object> cacheValues = redisTemplate.opsForValue().multiGet(cahcePageKeys);
            ...
            List<${className}> list = JSONArray.parseArray(JSON.toJSONString(cacheValues), ${className}.class);
                cachePageBean.setList(list);
                return cachePageBean;
            }
        }
        PageBean<${className}> dbPageBean = super.selectPage(query);
    ...
    public PageBean<${className}> cacheSelectPage(${className} query, List<String> fieldList) {
        ...
        PageBean<${className}> cachePageBean = (PageBean<${className}>)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cachePageBean)){
            return cachePageBean;
        }
        PageBean<${className}> dbPageBean = super.selectPage(query, fieldList);
    ...
    public Long cacheSelectCount(${className} query) {
        ...
        if(Objects.nonNull(cacheCount)){
            log.info("Cache Class = {}, Entity = {} Count",${className}.class.getName(),query);
            return cacheCount;
        }
