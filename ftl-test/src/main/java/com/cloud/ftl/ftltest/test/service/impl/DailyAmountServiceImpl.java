...
public class DailyAmountServiceImpl implements IDailyAmountService {
    ...
    */
    ...
    public void saveDailyAmountList(List<DailyAmount> list) throws Exception {
        ...
        List<DailyAmount> addList = new ArrayList<>();
        List<DailyAmount> updateList = new ArrayList<>();
        for (DailyAmount item : list){
            if(Objects.isNull(item.getDaId())){
                item.setDaId(dailyAmountRedis.getDailyAmountId());
                addList.add(item);
            }else{
                updateList.add(item);
            }
